...
def find_wechat_window():
    ...
    if not hwnd:
        print("[错误] 未找到微信窗口，请先打开微信并进入目标聊天窗口。")
        sys.exit(1)
    return hwnd
...
def calc_chat_region(window_rect, offset_left=280, offset_top=65, offset_bottom=120):
...
def main():
    parser = argparse.ArgumentParser(description="微信聊天记录自动化截屏工具")
    parser.add_argument("--output-dir", type=str, default="./output_wechat", help="截图输出目录（默认: ./output_wechat）")
    ...
    parser.add_argument("--offset-left", type=int, default=280, help="聊天区域左侧偏移像素（默认: 280）")
    parser.add_argument("--offset-top", type=int, default=65, help="聊天区域顶部偏移像素（默认: 65）")
    parser.add_argument("--offset-bottom", type=int, default=120, help="聊天区域底部偏移像素（默认: 120）")
    ...
    print(f"[信息] 找到微信窗口: 位置({window_rect[0]}, {window_rect[1]}) "
    ...
    print("\n[提示] 程序将在 3 秒后开始，请确保微信窗口已打开并定位到目标聊天...")
